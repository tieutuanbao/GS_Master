...
static uint8_t hid_init(usb_dev *udev, uint8_t config_index) {
    ...
    usbd_ep_recev (udev, CUSTOM_HID_EPOUT_ADDR, hid_handler.data, CUSTOM_HID_EPOUT_SIZE);
...
static uint8_t hid_req(usb_dev *udev, usb_req *req) {
    ...
    case GET_IDLE:
        ...
        transc->remain_len = CUSTOM_HID_EPOUT_SIZE;
        break;
    case GET_PROTOCOL:
        transc->xfer_buf = hid->data;
        transc->remain_len = CUSTOM_HID_EPOUT_SIZE;
...
static uint8_t hid_data_out(usb_dev *udev, uint8_t ep_num) {
    ...
    usbd_ep_recev(udev, CUSTOM_HID_EPOUT_ADDR, hid->data, CUSTOM_HID_EPOUT_SIZE);
    usbd_ep_send(udev, CUSTOM_HID_EPIN_ADDR, hid->data, CUSTOM_HID_EPIN_SIZE);
    return USBD_OK;
}